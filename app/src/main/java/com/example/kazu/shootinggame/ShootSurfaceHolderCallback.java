...
public class ShootSurfaceHolderCallback implements SurfaceHolder.Callback, Runnable {
    ...
    public ShootSurfaceHolderCallback(){
        ...
        paintlife = new Paint();
        paintlife.setStyle(Paint.Style.FILL);
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        MainActivity.bgimage = Bitmap.createScaledBitmap(MainActivity.bgimage,
                vieww, viewh, true);
        // 長方形の初期化
    ...
    public void drawOnCanvas(Canvas canvas){
        ...
        if(isShake){
            switch (bgimagex){
                case 0:
                    bgimagex = 5;
                    MainActivity.vibrator.vibrate(16*3); // 実機を振動
                    break;
                case 5:
                    bgimagex = -5;
                    break;
        ...
        canvas.drawBitmap(MainActivity.bgimage, bgimagex, 0, null);

        // 敵機描画
        canvas.drawBitmap(MainActivity.enemy,
        ...
        for(NwayBullet bullet: bullet1){
            canvas.drawBitmap(MainActivity.bullet1, bullet.x, bullet.y, null);
        }
        for(SnipeBullet bullet: bullet2){
            canvas.drawBitmap(MainActivity.bullet2, bullet.x, bullet.y, null);
        }

        if(spaceship.life > 0){
            // 自機描画
            canvas.drawBitmap(MainActivity.spaceship,
                    spaceship.x, spaceship.y, null);
            // 自弾描画
            canvas.drawBitmap(MainActivity.mybullet,
